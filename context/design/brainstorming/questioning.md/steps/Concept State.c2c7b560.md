...
content_id: c2c7b5606085e9512330a0ac9ae280fb8fb2cb98eb55104d76cc1e4489b77a6c
---
...
Below is a rough draft/skeleton of a CallConcept. I want to make sure that it is sufficiently modular and I would love your thoughts on how to best model CallWindows. The idea is that for every unique (User,Day) pair there is an CallSession. A user can then indicate that they are available to be called between a start time and and end time, a CallWindow. Importantly there can be multiple of these a day, and while a user has not completed their CallSession for that day, whenever an available a call window opens(with staggered retries depending on how recently we tried to call the user last) we call the user to see if we can record a journal entry. I also would like CallWindows to be separate enough that a user might be able to set come generic call windows week over week without us having to update every CallSession to make the necessary changes to the generic call windows. It would also be helpful if the model facilitates easy additional call windows outside of this generic setting to allow a more fine grained approach.

I would like concise notes that I will be able to further refine the concept with on my own.

I wold also like similar notes on the set of actions I have listed.

Below is a rough draft/skeleton of a CallConcept.

\<concept\_spec>
concept CallConcept\[User,JournalEntry]
...
# Users can have multiple call windows where they are able to complete a CallSession
...
# Each user can only have one call session per day
a set of CallSessions with
  a user User
  a date Date
  a numRetries Number
  a manuallyTriggered Boolean
  # user fails to pick up the call
  a missed Boolean
...
createCallSession(user:User, journalEntry: JournalEntry, date:Date)
deleteCallSession(user:User, date: Date)
triggerCallSession(user:User, date: Date)
retryCallSession(user:User, date:Date)
addCallWindow(user:User, date: Date, start: Date, end:Date)
deleteCallWindow(user:User, date:Date, start:Date)
updateCallWindow(user:User, date: Date, start:Date, endDate)
